	...
	const sendForm = () => {
		const message = {
			...
			ajaxHandler = {
				...
				error(status) {
					...
					console.warn(status.code + ' ' +  status.text);
				}
			},
			forms = [...document.forms];
		message.status.style.cssText = 'font-size: 2rem';
		message.status.style.color = '#fff';
		...
		forms.forEach(form => {
			...
			form.addEventListener('input', e => {
				...
				if (target.tagName.toLowerCase() === 'input' && !target.classList.contains('form-phone')) {
					if (target.value !== '') {
